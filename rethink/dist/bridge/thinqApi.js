...
export function signInUrl(baseUrl, countryCode) {
...
export class RemoteError extends Error {
    constructor(url, resultCode, result) {
...
export class Client {
    constructor(env) {
        ...
        if (Client.gatewayCache[env.countryCode] !== undefined)
            ...
            Client.gatewayCache[env.countryCode] = this.gateway = apiFetch(GATEWAY_URL, {
    ...
    get clientId() {
        return this.headers['x-user-no'] ?? 'rethink-monitor';
    }
    async auth(refreshToken) {
        ...
        const profile = await OAuth2.signedRequest(authUrl + '/users/profile', {
        ...
        const { item: homes } = await apiFetch(`${thinq2Uri}/service/homes`, { headers: this.headers });
    ...
    async listDevices() {
        ...
        const home = await apiFetch(`${thinq2Uri}/service/homes/${this.homeId}`, {
            headers: this.headers,
        });
        return home.devices;
    }
    ...
    async prepareNewT2Device() {
        const { thinq2Uri } = await this.gateway;
        return await apiFetch(`${thinq2Uri}/service/devices/otp/certificate`, {
            headers: this.headers,
            body: '{}',
            method: 'POST',
        });
    ...
    async addDevice(device, alias, deviceType, ciphertext) {
        if (!this.homeId)
            throw new Error('Current home is not set');
        const { thinq2Uri } = await this.gateway;
        const body = {
...
Client.gatewayCache = {};
export class Thinq1Device {
    constructor(deviceId, meta, state) {
        this.deviceId = deviceId;
        this.meta = meta;
        this.state = state;
        this.platformType = 'thinq1';
    }
}
export class Thinq2Device {
    constructor(deviceId, meta, state) {
        this.deviceId = deviceId;
        this.meta = meta;
        this.platformType = 'thinq2';
        this.nonce = randomBytes(8);
        this.state = state;
    }
    async pair(env, otpResponse) {
        log('bridge', 'fetching API urls');
        const servers = await apiFetch(`${IOT_BASE_URL}/route`, {
            headers: { 'x-country-code': env.countryCode, 'x-service-phase': 'OP', accept: 'application/json' },
        });
        log('bridge', 'fetching CA cert');
        // DEV call
        const { certificatePem: ca } = await apiFetch(`${IOT_BASE_URL}/route/certificate?name=aws-iot`, { headers: { accept: 'application/json' } });
        ...
        const deviceConfig = await apiFetch(`${servers.apiServer}/device/${this.deviceId}/certificate`, {
            method: 'POST',
        ...
        return publicEncrypt({ key: otpResponse.publicKey, padding: RSA_PKCS1_PADDING }, Buffer.concat([this.nonce, createHash('sha256').update(this.deviceId).digest()]));
    }
}
