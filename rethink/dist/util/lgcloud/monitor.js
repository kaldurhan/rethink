...
import { Client, IOT_BASE_URL, apiFetch, signInUrl } from '../../bridge/thinqApi.js';
...
async function oauth2Login(client) {
    ...
    let code = '';
    while (true) {
        const outUrl = await new Promise((resolve) => terminal.question('Paste the post-login URL here: ', resolve));
        try {
...
async function generateSubscription(client, log) {
    ...
    const response = await apiFetch(`${thinq2Uri}/service/users/client/certificate`, {
    ...
    log(`dbg: certificate received (${response.certificatePem.split('\n').length} lines), ${response.subscriptions.length} subscription topic(s)`);
    for (const t of response.subscriptions)
        log(`dbg: topic: ${t}`);
    return {
        key: privateKey,
...
async function openMQTT(client, subscription, opts) {
    ...
    const clientId = client.headers['x-client-id'];
    ...
    mqttClient.on('packetsend', (p) => log(`pkt-send: ${p.cmd}`));
    mqttClient.on('packetreceive', (p) => {
        if (p.cmd === 'connack') {
        ...
        for (const topic of subscription.subscriptions) {
            mqttClient.subscribe(topic, { qos: 1 }, (err) => {
                if (err)
                    ...
                    log(`dbg: subscribed to ${topic}`);
            });
        }
    });
    mqttClient.on('error', (err) => log(`error: ${err.code ?? ''} ${err.message}`));
    mqttClient.on('close', () => {
        const s = mqttClient.stream;
    ...
    if (stream) {
        stream.on('error', (err) => log(`tls-err: ${err.code ?? ''} ${err.message}`));
        stream.on('secureConnect', () => {
            ...
            if (peerCert?.subject?.CN) {
                log(`dbg: server cert CN=${peerCert.subject.CN} | issuer=${peerCert.issuer?.O ?? '?'} | expires=${peerCert.valid_to ?? '?'}`);
            }
        });
        ...
        stream.socket?.on('close', (hadError) => log(`dbg: TCP socket closed hadError=${hadError}`));
    }
    else {
...
async function promptCountryCode() {
    ...
    try {
        let cc = '';
        while (!/^[A-Z]{2}$/.test(cc)) {
            cc = (await new Promise((resolve) => terminal.question('Enter the 2-letter country code (e.g. US) matching your LG account: ', resolve)))
                .trim()
                .toUpperCase();
        }
        return cc;
...
export async function createSubscription(state, log) {
    const noop = log ?? (() => { });
...
// the certificate.
export async function connectWithSubscription(state, subscription, opts) {
    const client = new Client({ countryCode: state.countryCode });
    await client.auth(state.refreshToken);
...
export async function connect(state, opts) {
    const log = opts.log ?? (() => { });
    const client = new Client({ countryCode: state.countryCode });
    await client.auth(state.refreshToken);
