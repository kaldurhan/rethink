...
const CYCLE_IDS = Object.fromEntries(Object.entries(CYCLES).map(([k, v]) => [v, Number(k)]));
...
const DRY_LEVEL_IDS = Object.fromEntries(Object.entries(DRY_LEVELS).map(([k, v]) => [v, Number(k)]));
...
const ECO_HYBRID_IDS = Object.fromEntries(Object.entries(ECO_HYBRID).map(([k, v]) => [v, Number(k)]));
...
const RESERVATION_IDS = Object.fromEntries(Object.entries(RESERVATION).map(([k, v]) => [v, Number(k)]));
// Flag bits
const FLAG14_ANTI_CREASE = 0x02; // Bd[14] ✓
const FLAG15_REMOTE_START = 0x01; // Bd[15] ✓
const DOWNLOADED_CYCLES = {
...
export default class Device extends AABBDevice {
    constructor(HA, thinq, meta) {
        super(HA, thinq);
        this.lastDownloadedCycleId = 0; // tracks last known SmartCourse ID for wake sequence
        this.cycleStartTime = null; // set when state transitions to Running
        // Last known Bd values — used as fallback for start payload missing fields
        this.lastBdCycle = 0;
        this.lastBdDryLevel = 0;
        this.lastBdEcoHybrid = 0;
        this.lastBdAntiCrease = false;
        this.lastBdReservation = 0;
    ...
    processAABB(buf) {
        ...
        if (downloadedCycleId)
            this.lastDownloadedCycleId = downloadedCycleId;
        ...
        this.publishProperty('process_state', isOff ? '-' : (PROCESS_STATES[processState] ?? `unknown (${processState})`));
        ...
        this.publishProperty('downloaded_cycle_id', downloadedCycleId
    ...
    // ── Command builder ───────────────────────────────────────────────────────
    buildStartCommand(cycleId, dryLevel, ecoHybrid, antiCrease, reservation) {
    ...
    setProperty(prop, mqttValue) {
        ...
        if (prop === 'start') {
            ...
            if (mqttValue.trim()) {
                try {
                    const payload = JSON.parse(mqttValue);
                    if (payload.cycle !== undefined) {
                        const id = CYCLE_IDS[payload.cycle];
                        if (id !== undefined)
                            cycleId = id;
                        else
                    ...
                    if (payload.reservation !== undefined) {
                        const id = RESERVATION_IDS[payload.reservation];
                        if (id !== undefined)
                            reservation = id;
                        else
