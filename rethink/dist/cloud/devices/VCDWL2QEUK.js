 ...
 */
function findPowerBlock(inner) {
    let found = -1;
    for (let i = 0; i <= inner.length - 16; i++) {
        if (inner[i] === 0x10 && inner[i + 1] === 0x08)
            found = i;
    }
    return found;
}
 ...
 */
function findStatusSubBlock(inner) {
    for (let i = inner.length - 21; i >= 14; i--) {
        const marker = inner[i];
        if (marker !== 0x05 && marker !== 0x03 && marker !== 0x00)
            continue;
        const cs = inner[i + 4];
        if (cs === 0x00)
            continue;
...
export default class Device extends AABBDevice {
    ...
    processAABB(inner) {
        ...
        if (st === 0xec && sub && decodePhase(sub[1], sub[2]) === 'Finished')
            return;
        ...
        const isSelectionBlock = st === 0xec && sub !== null && inner[subStart + 20] === 0x01 && inner[subStart + 21] === 0x00;
