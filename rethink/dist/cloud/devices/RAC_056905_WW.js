...
export default class Device extends TLVDevice {
    constructor(HA, thinq, meta) {
        super(HA, thinq);
        this.initialValuesReceived = false;
        this.powerChangeHooks = [];
        this.modeChangeHooks = [];
        this.airClean = false;
        this.jetMode = false;
        this.energySave = false;
        this.filterUsedTime = 0;
        this.filterLifeTime = 0;
        this.filterChangedDate = 0;
        this.meta = meta;
    }
    drop() {
    ...
    valuesReceived() {
        ...
        // unref so this init-sequencing delay never holds the process open
    ...
    initProbeForFilter() {
        ...
        // unref so this filter-probe fallback never holds the process open
        // (the daemon is kept alive by its sockets; tests exit cleanly)
    ...
    publishFilterData() {
        const changedDate = Math.floor(this.filterChangedDate / 10000)
            .toString()
            .padStart(4, '0') +
            '-' +
    ...
    processFilterCmdResp(success, data) {
        if (!success) {
    ...
    publishClimateAction() {
        ...
        if (action != null)
            this.HA.publishProperty(this.id, 'climate-action', action);
    }
    getPowerTLV() {
    ...
    initMakeSetConfig() {
        const config = allowExtendedType({
        ...
        this.addField(config, {
            ...
            read_callback: (val) => {
                ...
                if (this.powerStatePrev !== powerState)
                    for (const hook of this.powerChangeHooks)
                        hook();
        ...
        if (this.raw_clip_state[0x2cc] & 1) {
            this.addModeDependentConfigSwitchField(config, 0x20f, 'airclean', 
            /* Same desc as in lg_thinq */
            'Air purify', 'mdi:air-purifier', 'airClean');
        }
        const jetCool = !!(this.raw_clip_state[0x2cd] & 1);
        const jetHeat = !!(this.raw_clip_state[0x2cd] & 2);
        if (jetCool || jetHeat) {
        ...
        if (this.raw_clip_state[0x2cc] & 2) {
            ...
            this.addModeDependentConfigSwitchField(config, 0x20d, 'energysave', 'Energy saving', 'mdi:flower', 'energySave', (mode) => mode === 0);
        }
    ...
    addTimerField(config, id, name, desc, icon, max) {
        const comp = {
            platform: 'number',
    ...
    addJetField(config, id, name, desc, icon, jetCool, jetHeat) {
        ...
        this.addField(config, {
            ...
            read_xform: (raw) => {
                if (jetCool && this.getModeTLV() === 0 && raw == 1)
                    return 'ON';
                if (jetHeat && this.getModeTLV() === 4 && raw == 2)
                    return 'ON';
                return 'OFF';
            },
            read_callback: (val) => {
                ...
                if (powerTLV === 0 || powerTLV == null)
                    return false;
                // Ignore read value if not in the right mode
                if (!((jetCool && this.getModeTLV() === 0) || (jetHeat && this.getModeTLV() === 4)))
                    return false;
                this.jetMode = val === 'ON';
                return true;
            },
    ...
    addOptionalSensorField(config, id, name, desc, icon, extra, read_xform) {
    ...
    addConfigSwitchField(config, id, name, desc, icon) {
        const comp = {
            platform: 'switch',
            unique_id: '$deviceid-' + name,
            name: desc,
    ...
    addModeDependentConfigSwitchField(config, id, name, desc, icon, field_name, check_mode) {
        const comp = {
            platform: 'switch',
            unique_id: '$deviceid-' + name,
            name: desc,
        ...
        this.addField(config, {
            ...
            read_callback: (val) => {
                ...
                if (powerTLV === 0 || powerTLV == null)
                    return false;
                // Ignore read value if not in the right mode
                if (!!check_mode && !check_mode(this.getModeTLV()))
                    return false;
