...
import { packStatus, unpackStatus } from './fridge_common.js';
...
function fridgeRange(temperatureUnit) {
...
function freezerRange(temperatureUnit) {
    if (temperatureUnit === 'F') {
        return {
            unit_of_measurement: '°F',
            min: -17,
...
function convertFridgeTemperature(temperatureUnit, input) {
    if (temperatureUnit === 'F')
        return 47 - input;
    else
        return 8 - input;
}
function convertToFreezerTemperature(temperatureUnit, input) {
    if (temperatureUnit === 'F') {
        if (input < -15)
            return 15;
        if (input < -12)
...
function convertFromFreezerTemperature(temperatureUnit, input) {
    if (temperatureUnit === 'F') {
...
export default class Device extends AABBDevice {
    ...
    processStatus(curStatus) {
        ...
        this.publishProperty('freezer_setpoint', convertFromFreezerTemperature(this.temperatureUnit, s.freezerSetpoint));
