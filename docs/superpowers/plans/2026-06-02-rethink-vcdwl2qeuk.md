 ...
 *   05 PH_A PH_B SP CS 00 00 00 00 00 00 00 00 00 TT_LO 00 TT_DUP 00 00 CS 01
 ...
 * STATUS sub-block is always the last one (21 bytes). We scan from the end
 * backwards for the unique signature: 0x05 at the start, repeated CS byte
 * at +19, 0x01 at +20.
 */
...
locator keyed on (0x05 marker, 0x01 terminator at +20, CS repeat
at +19). Sub-block is 21 bytes (packet-verified; plan originally had +21/+20). Decodes course, spin, temp, and Idle cycle_phase for the
...
Expected: all new tests pass. The boot-up packet test confirms the locator picks offset 64. The temp/spin tests assert the values the _last_ sub-block actually contains (which, during scroll-transition packets, lags the user's UI target by one step — that asymmetry is documented in spec § 10).
